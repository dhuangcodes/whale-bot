...
def get_leaderboard(limit: int = 300) -> list[dict]:
    ...
    Tries multiple parameter combinations since the API is finicky.
    Each entry has: proxyWallet, pnl, volume, name (optional)
    """
    # Try different param combos — Polymarket's leaderboard API is inconsistent
    attempts = [
        {"limit": limit, "window": "all"},
        {"limit": limit, "window": "30d"},
        {"limit": limit},
        {"n": limit, "window": "all"},
    ]

    for params in attempts:
        data = _get(f"{DATA}/leaderboard", params)
        if not data:
            continue

        # Handle different response shapes
        if isinstance(data, list) and data:
            log.info(f"Leaderboard: got {len(data)} entries (params={params})")
            return data
        if isinstance(data, dict):
            for key in ("data", "leaderboard", "results", "traders"):
                entries = data.get(key)
                if entries and isinstance(entries, list):
                    log.info(f"Leaderboard: got {len(entries)} entries via key={key}")
                    return entries

        log.debug(f"Leaderboard attempt {params} returned: {str(data)[:100]}")

    log.error("All leaderboard attempts failed")
    return []
